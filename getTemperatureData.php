...
$rows_json = mysql_query("SELECT time, temp FROM Presby WHERE id = '".$pid."' AND temp > 0 AND TIME_TO_SEC(timediff(time, '".$mysqldate."'))/(3600*24) > -5 AND TIME_TO_SEC(timediff(time, '".$mysqldate."'))/(3600*24) <= 0");
