...
$rows_json = mysql_query("SELECT time, lactate FROM Presby WHERE id = '".$pid."' AND lactate > 0 AND TIME_TO_SEC(timediff(time, '".$mysqldate."'))/(3600*24) > -2 AND TIME_TO_SEC(timediff(time, '".$mysqldate."'))/(3600*24) <= 0");
